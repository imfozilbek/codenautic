...
CodeNautic — AI-powered code intelligence platform. Ядро — автоматический анализ merge requests через 20-stage pipeline
с LLM. Помимо code review: визуализация кодовой базы (CodeCity), архитектурный анализ, предсказание проблемных зон,
...
3. Прогоняет через 20-stage pipeline с AI-анализом
...
flowchart TD
    ...
    subgraph cs["CodeNautic — 9 PM2-процессов"]
        ...
        subgraph core_layer["Core — Domain"]
            subgraph pipeline["20-Stage Pipeline"]
                P1["Validate -> Fetch Diff -> Fetch Context -> Chunk"]
                P2["Analyze -> Detect -> Filter -> Score -> Rank"]
                P3["Format -> Post Comments -> Emit Events"]
            end
...
### 1. 20-Stage Review Pipeline

Каждый merge request проходит через pipeline из 20 стадий с immutable context (обновления через Immer):
...
| Камеры раковины                   | Слои анализа: 20-stage pipeline                     |
