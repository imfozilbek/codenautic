...
| WORKER-INFRA-001 | Реализовать DLQ configuration | DONE | Реализовано | Реализация: Добавлен `BullMqDlqManager` (`src/worker/bullmq-dlq-manager.impl.ts`) с DLQ threshold `maxAttempts` (default `5`), event-based обработкой `failed` jobs, structured alert payload (`IWorkerDlqAlert`) и manual retry API (`retry(jobId)`). `BullMqQueueService` расширен default `attempts` configuration (`DEFAULT_WORKER_MAX_ATTEMPTS`) для корректного перехода jobs в failed/DLQ после 5 попыток. Добавлены новые контракты `IWorkerDlqManager`/`IWorkerDlqAlert`, DI-token `WORKER_TOKENS.DlqManager`, optional wiring в `registerWorkerModule`, обновлены exports и foundation wiring test. Добавлены тесты `tests/worker/bullmq-dlq-manager.impl.test.ts`, обновлены `tests/worker/bullmq-queue-service.impl.test.ts`. Готово, если: после исчерпания retry job уходит в DLQ с полной диагностикой, manual retry возвращает job в рабочую очередь без потери контекста; DoD: `cd packages/adapters && bun run lint && bun run typecheck && bun test`. |
