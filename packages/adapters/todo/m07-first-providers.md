...
|----------|--------------------|--------|-----------|---------------------|
| GIT-001 | Реализовать GitProviderFactory | TODO | Не начато | Реализация: Создает провайдер по типу. Возвращает IGitProvider. Инъекция токена. Готово, если: factory выбирает провайдера строго по конфигурации, неизвестный тип возвращает типизированную ошибку, контракт IGitProvider идентичен для всех поддерживаемых платформ; DoD: `cd packages/adapters && bun run lint && bun run typecheck && bun test`. |
