...
type IOpenAIClientMockOverrides = {
    readonly chatCreate?: IOpenAIClient["chat"]["completions"]["create"]
    readonly embedCreate?: IOpenAIClient["embeddings"]["create"]
}
...
/**
 * Creates default unexpected method for OpenAI mock sections.
 *
...
function createUnexpectedMethod<TMethod>(): TMethod {
    return ((..._args: readonly unknown[]): Promise<never> => {
        return Promise.reject(new Error("Unexpected OpenAI client call"))
    }) as TMethod
}
...
/**
 * Creates OpenAI client mock.
 *
 * @param overrides Partial method overrides.
 * @returns OpenAI-compatible mock.
 */
function createOpenAIClientMock(overrides: IOpenAIClientMockOverrides): IOpenAIClient {
    return {
        chat: {
            completions: {
...
describe("OpenAIProvider", () => {
    test("executes chat request with tools, finish reason and JSON mode", async () => {
        ...
        const provider = new OpenAIProvider({
            client: createOpenAIClientMock({
                chatCreate: chatCreate as unknown as IOpenAIClient["chat"]["completions"]["create"],
            }),
        })
    ...
    test("stops on non-retryable errors and exposes exhausted retry metadata", async () => {
        const badRequestProvider = new OpenAIProvider({
            client: createOpenAIClientMock({
                chatCreate: createQueuedAsyncMethod([
                    () => {
                        throw APIError.generate(400, {}, "bad request", new Headers())
                    },
                ]) as unknown as IOpenAIClient["chat"]["completions"]["create"],
            }),
        })
        ...
        expect(badRequestError).toBeInstanceOf(OpenAIProviderError)
        ...
        const sleepCalls: number[] = []
        const serverProvider = new OpenAIProvider({
            client: createOpenAIClientMock({
                chatCreate: createQueuedAsyncMethod([
                    () => {
        ...
        expect(serverError).toBeInstanceOf(OpenAIProviderError)
